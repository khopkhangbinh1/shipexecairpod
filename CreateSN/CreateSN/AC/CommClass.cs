...
namespace CreatePPSDataAC
{
    class CommClass
    {
        ...
        public string SN_InsertWo2(string sn, string csn, string wo, string partid, string cartonno, string locationid, string locationNo, string palletno, string strdn, string strdnline, string strcarlineno, string strPoint)
        {
            object[][] procParams = new object[13][];
            procParams[0] = new object[] { ParameterDirection.Input, OracleType.VarChar, "sn", csn+"01" };
        ...
        public bool SN_InsertWo(string sn, string csn, string wo, string partid, string cartonno, string locationid, string PALLET_NO)
        {
         ...
         VALUES ( :WORK_ORDER,
            ...
            new object[] { ParameterDirection.Input, OracleType.VarChar, "SERIAL_NUMBER", csn +"01" },
        ...
        public bool ACSN_InsertWo(string sn, string csn, string wo, string partid, string cartonno, string locationid, string PALLET_NO)
        {
            try
            {
                string sqlstr = @"INSERT INTO NONEDIPPS.G_SN_STATUS (WORK_ORDER,
                                   PART_ID,
                                   SERIAL_NUMBER,
                                   CARTON_NO,
                                   CONTAINER,
        ...
        public bool CheckPartNo(string partno, out string partid)
        {
            try
            {
                string sqlstr = "SELECT MAX(PART_ID) as PART_ID FROM SAJET.SYS_PART WHERE PART_NO=:PART_NO AND PART_NO IS NOT NULL";
        ...
        public bool ACCheckLocationId(string locationno, out string locationid)
        {
            try
            {
                string sqlstr = "SELECT LOCATION_ID FROM NONEDIPPS.WMS_LOCATION WHERE LOCATION_NO=:LOCATION_NO AND ROWNUM=1";
                object[][] sqlparams = new object[][] { new object[] { ParameterDirection.Input, OracleType.VarChar, "LOCATION_NO", locationno } };
                DataTable dt = ClientUtils.ExecuteSQL(sqlstr, sqlparams).Tables[0];
                if (dt.Rows.Count > 0)
                {
        ...
        public string ACgetGTIN(string ictpartno, out string strUPC, out string strJAN, out string strCustModel, out string strSCC14)
        {
            //string strSQL = string.Format("SELECT  a.part_no ,decode(a.jan_code, '', a.upc, a.jan_code) as gtin  "
            //                              + "   FROM sajet.sys_part a "
            //                              + "  where a.part_no = '{0}' "
        ...
        public string ACgetMPNInfo(string Partno)
        {
            string strSql = string.Format("select  trim(ICTPARTNO||'*'||MPN||'*'||PACKUNIT||'*'||max(TOTALQTY) )  as MPN1  "
                            + "  from NONEDIPPS.vw_mpn_info "
                            + " where  ICTPARTNO ='{0}' group by  ICTPARTNO, MPN ,PACKUNIT  ", Partno);
            //+" where  ( packcode <> subpackcode or subpackcode is null) and ICTPARTNO ='{0}' ", Partno);
        ...
        public string ACcheckLocation(string LocationNo)
        {
            //已经存在于t_location return "NG" 
            string strSql = string.Format("select  *  "

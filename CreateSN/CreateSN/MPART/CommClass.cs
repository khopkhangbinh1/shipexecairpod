...
namespace CreatePPSData
{
    class CommClass
    {
        ...
        public bool SN_InsertWo(string sn, string csn, string wo, string partid, string cartonno, string locationid, string PALLET_NO, string strCoo)
        {
            try
            {
     ...
     INNER JOIN PPSUSER.WMS_LOCATION B
        ON to_char(A.RC_NO) = to_char(B.LOCATION_ID)
     INNER JOIN SAJET.SYS_PART C
     ...
     WHERE   A.serial_number=:SERIAL_NUMBER";
        ...
        public string getGTIN(string ictpartno, out string strUPC, out string strJAN, out string strCustModel, out string strSCC14)
        {
            ...
            string strSQL = string.Format("select a.ictpartno, "
                                     ...
                                     + "   where  (a.packcode <> subpackcode or a.subpackcode is null) and  a.MPN like 'M%' and a.ictpartno = '{0}' ", ictpartno);

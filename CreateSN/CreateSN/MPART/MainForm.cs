...
namespace CreatePPSData
{
    public partial class MainForm : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //ClientUtils.ServerUrl = "http://172.23.28.52:8090/WCF_RemoteObject";//  光州
            //KS ap server:
        ...
        private void initComboBox()
        {
            ...
            //                + " order by MPN1 ";
            string strSql = @"
                                 select trim(ICTPARTNO || '*' || MPN || '*' || PACKUNIT || '*' ||
                                                  max(TOTALQTY))   MPN1
                                        from ppsuser.vw_mpn_info where MPN like 'M%'
                                       group by ICTPARTNO, MPN, PACKUNIT
                                       order by MPN1
                                ";
            ...
            DataSet dts3 = ClientUtils.ExecuteSQL("select cntycode2 from ppsuser.t_country_code_iso");
            if (dts3 != null && dts3.Tables[0].Rows.Count > 0)
            {
                cmbCoo.DataSource = dts3.Tables[0];
                cmbCoo.DisplayMember = "cntycode2";
                cmbCoo.ValueMember = "cntycode2";
            }
            else
            {
        ...
        private void btnCreateCarton_Click(object sender, EventArgs e)
        {
            ...
            string strResult = CreateCSN(strLOCATIONNO, strpartno, strCARTONCOUNT, strCSNTYPE, strCARTONTYPE, strCoo);
        ...
        public string CreateCSN(string locationno, string partno, string cartoncount, string csntype, string cartontype)
        {
            ...
            string PPP = cc.GetCoo(strpartno, 1);
            ...
            {
                //"SCCARTONA"   Franky Remark: SSCC18 from table :t_test_sn_range
            ...
            if (cartontype.Equals("SCCARTONA") || cartontype.Equals("SCCARTON"))
            {
                ...
                //edit by franky b298&b288 用同一款label
                if ((strCustModel.Equals("B501") || strCustModel.Equals("B188") || strCustModel.Equals("B298") || strCustModel.Equals("B288")) && (chkCSNListLabel.Checked))
                {
                    using (Process p = new Process())
                    {
                        string strSampleFile = string.Empty;
        ...
        public string CreateCSN(string locationno, string partno, string cartoncount, string csntype, string cartontype, string strCoo)
        {
            if (string.IsNullOrEmpty(partno))
            {
                MessageBox.Show("料号为空");
            ...
            for (int i = 1; i <= cartonscount; i++)
            {
                ...
                for (int j = 1; j <= qty; j++)
                {
                    ...
                    if (!cc.SN_InsertWo(strTemp1, CSN, strTemp2, partID, cartonno, locationID, palletNo, strCoo))
                    {
                        return "NG";
                    }
                    snlist += CSN + "|";

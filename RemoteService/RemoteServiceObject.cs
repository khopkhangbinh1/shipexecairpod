...
namespace RemoteService
{
    public class RemoteServiceObject : HttpHosting, IRemoteServiceObject
    {
        ...
        [Obsolete]
        public static bool tcpListen(string strip, string port)
        {
            if (!string.IsNullOrEmpty(port))
            {
                IPAddress ip = Dns.GetHostByName(strip).AddressList.FirstOrDefault();
        ...
        public void RefreshClientList()
        {
            ...
            {
                //尝试清除冗余信息
                foreach (var key in ConnStatics.ClientList.Keys)
                {
                    if (!tcpListen(key + ".luxshare.com.cn", "8901"))
                    {
                        ConnStatics.ClientList.Remove(key);
                    }
                }
            }
            catch { }

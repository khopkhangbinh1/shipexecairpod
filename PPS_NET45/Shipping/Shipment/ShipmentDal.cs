...
namespace Shipment
{
    public class ShipmentDal
    {
        ...
        public DataSet GetCarList(string startDate, string endDate)
        {
            ...
            {
                MessageBox.Show("获取卡车信息失败".TP() + e.ToString());
            }
        ...
        public void UpdatePalletFlagandWC(string palletNo)
        {
            try
            {
                ...
                //update wc 
                string stringupdatewc = @"update ppsuser.t_sn_status set wc ='W5',udt=sysdate where pack_pallet_no in (select pallet_no  from ppsuser.t_shipment_pallet where pallet_no =:palletNo  or  real_pallet_no=:palletNo )";
            ...
            {
                MessageBox.Show("更新数据异常".TP() + e.ToString());
            }
...
AND nvl(a.isload,0)<>1";
        ...
        #region update from js source by wenxing 2020-08-12
        /// <summary>
        /// 看是否有AC和EDI维护在同一辆车上
        /// </summary>
        ...
        {
            string strSql = @"SELECT DISTINCT a.CAR_NO FROM pptest.oms_load_car a WHERE a.pallet_no=:PALLET_NO";
            object[][] parameterArray = new object[1][];
        ...
        {
            string strSql = @"SELECT t.SORT 序列, t.CAR_NO 车牌号,t.SHIPMENT_ID 集货单 ,t.PALLET_NO 栈板号,t.EXPECTEDTIME,--t.ISLOAD,
                            case 
                            when t.loadtime is not null and t.ISLOAD='1' then '已装车'
                            else  '未装车'
                            end  装车状态 
                              FROM PPTEST.OMS_LOAD_CAR t
                             WHERE t.EXPECTEDTIME >= TRUNC (SYSDATE) AND t.CAR_NO = :carNo
                             order by t.SORT， t.PALLET_NO";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "carNo", carNo };
            return ClientUtils.ExecuteSQL(strSql, parameterArray).Tables[0];
        }

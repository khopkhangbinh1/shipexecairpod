...
namespace wmsReport.DataGetWays
{
    class GetDatas
    {
        ...
        public DataTable queryMoreInfoByMultiOption(string Mes_pallet, string pps_pallet, string pick_pallet, string deliveryNo, string locationNo, string ictPn, string cartonNo, string ssccCode, string shipmentId)
        {
            ...
            {
                addSql += @" AND  (TSS.CARTON_NO=:CartonNo or TSS.Customer_sn=:CartonNo or TSS.serial_number=:CartonNo) ";
                countIndex += 1;
                Array.Resize(ref sqlparams, sqlparams.Length + 1);
            ...
            string sql = @"      SELECT distinct   TSS.PALLET_NO AS MES_栈板号,
                                     ...
                                     TSS.LINE_ITEM AS LineItem,
                                     tss.part_no as 料号,
                                     tss.sscc, tss.TRACKING_NO, tss.BABYTRACKING_NO
                       FROM PPSUSER.T_SN_STATUS TSS
                      WHERE 
        ...
        public DataTable getAllLocationInfo(bool isType)
        {
            ...
            string addSql = @"  union
                                SELECT DISTINCT wl.location_id, WL.location_no
                                 ...
                                 WHERE WL.WAREHOUSE_ID in (SELECT WAREHOUSE_ID FROM SAJET.WMS_WAREHOUSE WHERE ENABLED='Y')
                                   AND WL.Location_No  in ( 'SYS','TEMPSTORE') ";
            ...
            sSQL = @"        SELECT DISTINCT wl.location_id, WL.location_no
                              ...
                              LEFT JOIN PPSUSER.T_LOCATION TL
                                ON TL.LOCATION_ID = WL.LOCATION_ID
                             WHERE WL.WAREHOUSE_ID in (SELECT WAREHOUSE_ID FROM SAJET.WMS_WAREHOUSE WHERE ENABLED='Y')
                               AND WL.Location_No  not in ( 'SYS','SOLIDSTORE','TEMPSTORE') " + isTransferSql + addSql;
            //object[][] sqlparams = new object[1][];
        ...
        /// <summary>
        /// 如果SYS储位在t_location 找不到就依照下面来检查出来相关信息 ADD BY JAMMY 20200829
        /// </summary>
        /// <param name="locationId"></param>
        /// <returns></returns>
        public DataTable getLocationInforBySYSlocationId(string locationId)
        {
            string sqlstr = @"  SELECT A.LOCATION_NO      AS 储位名,
                   A.PALLET_NO        AS 栈板号,
                   A.QTY              AS 总数量,
                   A.CARTONQQTY       AS 总箱数,
                   A.PART_NO          AS 料号,
                   B.QHTY             AS QAHold数量,
                   B.QHCARTONQQTY     AS QAHold箱数
              FROM (  SELECT tss.LOCATION_NO,
                             tss.PALLET_NO,
                             COUNT (tss.SERIAL_NUMBER)          QTY,
                             COUNT (DISTINCT tss.CARTON_NO)     CARTONQQTY,
                             tss.PART_NO
                        FROM t_sn_status TSS
                             INNER JOIN sajet.g_sn_status gss
                                 ON TSS.SERIAL_NUMBER = gss.SERIAL_NUMBER
                       WHERE tss.wc = 'W0' 
                    GROUP BY tss.PALLET_NO, tss.PART_NO, tss.LOCATION_NO) A,
                   (  SELECT tss.LOCATION_NO,
                             tss.PALLET_NO,
                             COUNT (GSS.SERIAL_NUMBER)          QHTY,
                             COUNT (DISTINCT GSS.CARTON_NO)     QHCARTONQQTY,
                             tss.PART_NO
                        FROM t_sn_status TSS,  sajet.g_sn_status gss
                       WHERE     tss.wc = 'W0'
                             AND TSS.SERIAL_NUMBER = gss.SERIAL_NUMBER
                             AND gss.HOLD_FLAG = 'Y'
                    GROUP BY tss.PALLET_NO, tss.PART_NO, tss.LOCATION_NO) B,
                    WMS_LOCATION WTL
             WHERE     A.LOCATION_NO = B.LOCATION_NO
                   AND A.PALLET_NO = B.PALLET_NO
                   AND A.PART_NO = B.PART_NO
                        AND a.LOCATION_NO = WTL.LOCATION_NO
                        AND wtl.LOCATION_ID = :LocationId ";
            object[][] sqlparams = new object[1][];
            sqlparams[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "LocationId", locationId };
            return ClientUtils.ExecuteSQL(sqlstr, sqlparams).Tables[0];
        }

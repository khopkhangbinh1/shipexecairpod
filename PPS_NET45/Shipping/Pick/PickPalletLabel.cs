...
namespace PickList
{
    class PickPalletLabel
    {
        ...
        public bool PrintPickPalletLabel(string strPickpalletno)
        {
            ...
            {
                MessageBox.Show("strSQL执行异常".TL() + ex.ToString());
                return false;
            }
            string strHead = "";
            if (dt.Rows.Count == 1)
            ...
            {
                MessageBox.Show("QTY2执行异常".TL() + ex2.ToString());
                return false;
            }
            if (dt2.Rows.Count > 0)
            {
            ...
            {
                MessageBox.Show("QTY1执行异常".TL() + ex3.ToString());
                return false;
            }
            if (dt3.Rows.Count > 0)
            {
            ...
            if (strPickpalletno.Substring(1, 1).Equals("9"))
            {
                ...
                {
                    //这么写不好，再改改。
                    MessageBox.Show("9号pickpallet,必须拣货完成自动打印".TL());
                    return false;
                }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < TOTALPAGE; i++)
                {
                    using (Process p = new Process())
                    {
                        ...
                        if (!File.Exists(strSampleFile))
                        {
                            MessageBox.Show("Sample File Not exists-".TL() + strSampleFile);
                            return false;
                        }
                        p.StartInfo.FileName = "bartend.exe";
                        string sArguments = @" /F=@PATH1 /D=@PATH2 /P /X /C=@QTY";
        ...
        public bool PrintPickPalletLabel_new(string strPickpalletno)
        {
            ...
            //HYQ： 这部分是写.dat文件。 5个栏位就好
            string LabelParam = @"SHIPMENT_ID|PICK_PALLET_NO|CARRIER|QTY|SHIPMENT_TYPE|QTY1|QTY2|TYPE|";
            ...
            string SQL = string.Format(@"
                                select c.shipment_id,
                                  ...
                                  from (select a.shipment_id,
                                               b.pick_pallet_no,
                                               (select distinct e.carrier_name
                                                  from pptest.oms_carrier_tracking_prefix d
                                                 where trim(d.carriercode) = e.carrier_code
                                                   and d.shipmode = e.transport
                                                   and d.region = e.region
                                 ...
                                 group by c.shipment_id,
                                          ...
                                          c.carrname,
                                          c.pallettype,
                                          c.remark,c.TYPE", strPickpalletno);
            DataTable dt = new DataTable();
            try
            {
                dt = ClientUtils.ExecuteSQL(SQL).Tables[0];
            ...
            {
                MessageBox.Show("SQL执行异常".TL() + ex.ToString());
                return false;
            }
            string strHead = "";
            ...
            strHead = strHead + str6 + "," + str7 + "," + strpackcodedesc + ","+ strtype+",";
            strHead = LabelParam + "\r\n" + strHead;
            ...
            //    return false;
            //}
            //ClientUtils.PrintBartender(strSampleFile, strLst, "ZDesigner ZT410-600dpi ZPL");
            return true;
        }

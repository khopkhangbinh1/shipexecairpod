...
namespace WeightAC
{
    public class WeightDal
    {
        ...
        public DataTable GetPalletUnitInfo(string strPalletNo)
        {
            object[][] sqlparams = new object[1][];
            sqlparams[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "PALLET_NO", strPalletNo };
            return ClientUtils.ExecuteSQL(@"SELECT DISTINCT a.PALLET_NO FROM NONEDIPPS.T_PALLET_ORDER a INNER JOIN NONEDIPPS.VW_MPN_INFO b 
ON a.ICTPN=b.ICTPARTNO 
WHERE a.PALLET_NO=:PALLET_NO AND b.CUSTMODEL='TIP' ", sqlparams).Tables[0];
        }
 ...
 INNER JOIN NONEDIPPS.T_PALLET_ORDER cc
    ON aa.shipment_id = cc.SHIPMENT_ID
 ...
 GROUP BY bb.PALLET_NO,
          ...
          cc.LINE_ITEM,
          cc.MPN
 ORDER BY cc.DELIVERY_NO,cc.LINE_ITEM, cc.MPN ", sqlparams).Tables[0];
        }

        public DataTable GetShippingMarkAppleCareInfo(string strPalletNo)
        {
            object[][] sqlparams = new object[1][];
            sqlparams[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "PALLET_NO", strPalletNo };
            return ClientUtils.ExecuteSQL(@"SELECT DISTINCT AA.HAWB,
                                           BB.PALLET_NO,
                                           BB.QTY,
                                           BB.CARTON_QTY,
                                           BB.WEIGHT,
                                           ...
                                           DD.SHIPTOCITY
                                      FROM NONEDIPPS.T_SHIPMENT_INFO   AA,
                                           NONEDIPPS.T_SHIPMENT_PALLET BB,
                                           NONEDIPPS.T_PALLET_ORDER    CC,
                                           NONEDIPPS.T_FD_ORDER_DETAIL DD,
                                           NONEDIPPS.T_SHIPPER         EE,
                                           NONEDIPPS.T_ORDER_INFO FF
                                     WHERE AA.SHIPMENT_ID = BB.SHIPMENT_ID
                                       AND AA.SHIPMENT_ID = CC.SHIPMENT_ID
                                       AND BB.PALLET_NO = CC.PALLET_NO
                                       AND CC.DELIVERY_NO = DD.AC_PO
                                       AND CC.LINE_ITEM = DD.AC_PO_LINE
                                       AND CC.SHIPMENT_ID=FF.SHIPMENT_ID
                                       AND CC.DELIVERY_NO=FF.DELIVERY_NO
                                       AND CC.LINE_ITEM=FF.LINE_ITEM
                                       AND CC.ICTPN=FF.ICTPN
                                       AND DD.FREIGHTORDER=FF.FREIGHTORDER
                                       AND BB.PALLET_NO = :PALLET_NO
                                     ORDER BY CC.DELIVERY_NO,CC.MPN ", sqlparams).Tables[0];
        }

        public DataTable getShippingInfoByPalletNo(string strPalletNo)
        {
            object[][] sqlparams = new object[1][];
            sqlparams[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "PALLET_NO", strPalletNo };
            return ClientUtils.ExecuteSQL(@"SELECT DISTINCT 
                                           EE.SHIPPERNAME,
                                           EE.SHIPPERADDRESS1,
                                           EE.SHIPPERADDRESS2,
                                           EE.SHIPPERCITY,
                                     ...
                                     WHERE AA.SHIPMENT_ID = BB.SHIPMENT_ID
                                       ...
                                       AND CC.ICTPN=FF.ICTPN
                                       AND DD.FREIGHTORDER=FF.FREIGHTORDER
                                       AND BB.PALLET_NO = :PALLET_NO ", sqlparams).Tables[0];
        }

...
namespace WeightAC
{
    public partial class fMain : Form
    {
                    ...
                    if ((WeightValue <= UpperWeight) && (LowerWeight <= WeightValue))
                    {
                        ...
                        if (dt.Rows[0]["errmsg"].ToString().Contains("OK"))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    lbl_WeightValue.Text = "0.00";
                                    Show_Message("打印PALLET LOADING SHEET OK", 1);
                                }
                                else
                                {
                                    lbl_WeightValue.Text = "0.00";
                                    Show_Message("打印PALLET LOADING SHEET连接出了问题", 0);
                                    return;
                                }
                                if (dspslm.PrintShippingMarkLabel(StrPalletNo, labellistrows, pages, out msg))
                                {
                                    Show_Message("打印ShippingMarkLabel OK", 1);
                                }
                                else
                                {
                                    if (!string.IsNullOrEmpty(msg))
                                    {
                                        if (msg.Contains("shipto") || msg.Contains("此栈板对应DN出货地址不唯一"))
                                        {
                                            Show_Message(msg, 0);
                                            return;
                                        }
                                    ...
                                    {
                                        Show_Message("打印ShippingMarkLabel连接出了问题", 0);
                                        return;
                                    }
                                }
                            }
        ...
        private void btn_reprintmark_Click(object sender, EventArgs e)
        {
            txtMessage.Text = "";
            txtMessage.BackColor = Color.Blue;
            fCheck fcheck = new fCheck();

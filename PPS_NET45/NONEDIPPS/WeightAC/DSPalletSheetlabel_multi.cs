    ...
    public class DSPalletSheetlabel_multi
    {
        ...
        public bool PrintPalletLabel(string strpalletno, int listrows, string pages)
        {
            ...
            if (dt_mixTotal.Rows.Count > 0)
            {
                ...
                //增加单位
                string strUnit = "pcs";
                DataTable dtTempAA = wb.GetPalletUnitInfo(strpalletno);
                if ((dtTempAA != null) && (dtTempAA.Rows.Count > 0))
                {
                    strUnit = "set";
                }
                //确定这两个值
        ...
        public bool PrintShippingMarkLabel(string strpalletno, int listrows, string pages, out string msg)
        {
            //---------------------
            msg = "";
            string sMessage = "";
            ...
            strLabelName = @"weight_ShippingMarkLabel_Appcare";
            listrows = 15;
            ...
            DataTable dt_mixTotal = new DataTable();
            dt_mixTotal = wb.GetShippingMarkAppleCareInfo(strpalletno);

            if (dt_mixTotal.Rows.Count > 0)
            {
                //判断shiptocity='SHANGHAI'才打印
                if (!wb.checkIsPrint(dt_mixTotal, out sMessage))
                {
                    msg = sMessage;
                    return false;
                }
                DataTable dt_dnlist = new DataTable();
                dt_dnlist = wb.getShippingInfoByPalletNo(strpalletno);
                if (dt_dnlist.Rows.Count > 1)
                {
                    msg = "此栈板对应DN出货地址不唯一，请联系IT-PPS";
                    return false;
                }
                //确定打印的label的张数palletcount， 定义个list的行数datarows，
                //mix的所有行数
                ...
                int TOTALPAGE = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(mixTotalrow) / Convert.ToDouble(listrows)));
                //HYQ： 这部分是写.dat文件。
                string LabelParam = @"CURPAGE|TOTALPAGE|HAWB|PALLET_NO|QTY|CARTON_QTY|WEIGHT|SHIPPERNAME|SHIPPERADDRESS1|SHIPPERADDRESS2|SHIPPERCITY";
                LabelParam = LabelParam + @"|SHIPPERCOUNTRY|SHIPTONAME1|SHIPTOADDRESS1|SHIPTOADDRESS2|SHIPTOADDRESS3|SHIPTOCITY";
                LabelParam = LabelParam + @"|PO1|PA1";
                LabelParam = LabelParam + @"|PO2|PA2";
                LabelParam = LabelParam + @"|PO3|PA3";
                LabelParam = LabelParam + @"|PO4|PA4";
                LabelParam = LabelParam + @"|PO5|PA5";
                LabelParam = LabelParam + @"|PO6|PA6";
                LabelParam = LabelParam + @"|PO7|PA7";
                LabelParam = LabelParam + @"|PO8|PA8";
                LabelParam = LabelParam + @"|PO9|PA9";
                LabelParam = LabelParam + @"|PO10|PA10";
                LabelParam = LabelParam + @"|PO11|PA11";
                LabelParam = LabelParam + @"|PO12|PA12";
                LabelParam = LabelParam + @"|PO13|PA13";
                LabelParam = LabelParam + @"|PO14|PA14";
                LabelParam = LabelParam + @"|PO15|PA15";
                ...
                for (int i = 0; i < TOTALPAGE; i++)
                {
                    ...
                    string strtotalqty = dt_mixTotal.Rows[i]["QTY"].ToString();
                    string strweight = Convert.ToDouble(dt_mixTotal.Rows[i]["WEIGHT"]).ToString("0.00");
                    string strcartonqty = dt_mixTotal.Rows[i]["CARTON_QTY"].ToString();
                    string strshippername = dt_mixTotal.Rows[i]["SHIPPERNAME"].ToString();
                    string strshipperaddress1 = dt_mixTotal.Rows[i]["SHIPPERADDRESS1"].ToString();
                    string strshipperaddress2 = dt_mixTotal.Rows[i]["SHIPPERADDRESS2"].ToString();
                    string strshippercity = dt_mixTotal.Rows[i]["SHIPPERCITY"].ToString();
                    string strshippercountry = dt_mixTotal.Rows[i]["SHIPPERCOUNTRY"].ToString();
                    string strshiptoname1 = dt_mixTotal.Rows[i]["SHIPTONAME1"].ToString();
                    string strshiptoaddress1 = dt_mixTotal.Rows[i]["SHIPTOADDRESS1"].ToString();
                    string strshiptoaddress2 = dt_mixTotal.Rows[i]["SHIPTOADDRESS2"].ToString();
                    string strshiptoaddress3 = dt_mixTotal.Rows[i]["SHIPTOADDRESS3"].ToString();
                    string strshiptocity = dt_mixTotal.Rows[i]["SHIPTOCITY"].ToString();
                    string allshipperInfo = wb.calculateShiptoORReturnTo(strshippername, strshipperaddress1, strshipperaddress2 + "," + strshippercity, strshippercountry);
                    string allshiptoinfo = wb.calculateShiptoORReturnTo(strshiptoname1, strshiptoaddress1, strshiptoaddress2, strshiptoaddress3, strshiptocity);
